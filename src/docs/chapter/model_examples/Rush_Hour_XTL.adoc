...
This is a link:/Other_languages[Prolog XTL encoding] of the
link:/Rush_Hour_Puzzle[B solution which is also available]. On the
...
extension so that it can be loaded by link:/Installation[ProB Tcl/Tk] or
...
/File:ProB_RushHour_XTL_Screenshot.png[600px|center]
...
link:/Using_the_Command-Line_Version_of_ProB[probcli] (run on a Mac Book
Air, 1.7 GHz i7):
